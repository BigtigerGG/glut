...
GMatrix3d & GMatrix3d::operator = (const GMatrix3d &m)
{
	for(int i=0; i<16; i++) mMatrix[i] = m.mMatrix[i];
	return *this;
}
...
GMatrix3d GMatrix3d::createRotate(double af, const GVector3d &v)
{
	GMatrix3d m;
	...
	//(1)绕Z轴旋转到YOZ平面
	m = createRotateZ(v.x()/lxy, v.y()/lxy);

	//(2)绕X轴旋转到ZOX平面
	m *= createRotateX(lxy/len, v.z()/len);
